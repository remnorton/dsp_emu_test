...
uint16_t dspDrawChar(uint8_t handle, uint32_t x, uint32_t y, Font_type_t* font, uint8_t sym, uint8_t direction)
{
    if (!handle) return 0;
    if (!dsp[handle-1].init->driver->drawBitmap) return 0;
    if (!font) return 0;
    ...
    dspDrawBitmap(handle,x,y+startY,&bmp,direction);
    free(bmp.data);
    return sym_width+font->spacing;
}

void dspDrawString(uint8_t handle, uint32_t x, uint32_t y, Font_type_t* font, uint8_t* text, uint8_t allign, uint8_t direction)
{
    ...
    while (pix_len > (vertical?dspGetScreenHeight(handle):dspGetScreenWidth(handle)))
    {
        ch_len--;
    ...
    {
        if (direction >= DSP_TextDir_RightLeft) pos += dspDrawChar(handle, vertical?sx:(sx+pos), vertical?(sy+pos):sy, font, text[ch_len-i-1], (direction==DSP_TextDir_RightLeft)?DSP_TextDir_LeftRight:direction);
        else pos += dspDrawChar(handle, vertical?sx:(sx+pos), vertical?(sy+pos):sy, font, text[i], (direction==DSP_TextDir_RightLeft)?DSP_TextDir_LeftRight:direction);
    }
...
void dspDrawBitmap(uint8_t handle, uint32_t x, uint32_t y, DSP_Bitmap_t* bmp, uint8_t angle_code)
{
    if (!handle) return;
    if (!dsp[handle-1].init) return;
    if (dsp[handle-1].init->driver->drawBitmap)
    {
        if (angle_code != DSP_Rotate_0)
        {
            uint32_t width = bmp->width;
            while (width % 8) width++;
            uint32_t shift = width - bmp->width;
            uint32_t s = width*bmp->height/8;
            DSP_Bitmap_t tmp = {bmp->width, bmp->height, 0};
            tmp.data = (uint8_t*)malloc(s);
            if (tmp.data)
            {
                memset(tmp.data, 0, s);
                //rotate bitmap
                switch(angle_code)
                {
                    case DSP_Rotate_MirrorX:
                    {
                        for (uint32_t y = 0; y < bmp->height; y++)
                            for (uint32_t x = 0; x < width; x++)
                                if (bmp->data[x/8+(y*width/8)] & (0x80 >> (x%8))) tmp.data[(width-shift-x-1)/8+(y*width/8)] |= (0x80 >> ((width-shift-x-1)%8));

                        break;
                    }
                    case DSP_Rotate_MirrorY:
                    {
                        for (uint32_t y = 0; y < bmp->height; y++)
                            for (uint32_t x = 0; x < width; x++)
                                if (bmp->data[x/8+(y*width/8)] & (0x80 >> (x%8))) tmp.data[x/8+((tmp.height-y-1)*width/8)] |= (0x80 >> (x%8));
                    ...
                    case DSP_Rotate_180:
                    {
                        for (uint32_t y = 0; y < bmp->height; y++)
                            for (uint32_t x = 0; x < width; x++)
                                if (bmp->data[x/8+(y*width/8)] & (0x80 >> (x%8))) tmp.data[(width-shift-x-1)/8+((tmp.height-y-1)*width/8)] |= (0x80 >> ((width-shift-x-1)%8));
                    ...
                    case DSP_Rotate_90:
                    {
                        ...
                        for (uint32_t x = 0; x < width; x++)
                            for (uint32_t y = 0; y < bmp->height; y++)
                               if (bmp->data[x/8+(y*width/8)] & (0x80 >> (x%8))) tmp.data[(tmp.width-y-1)/8+(x*width/8)] |= (0x80 >> ((tmp.width-y-1)%8));
                        break;
                    }
                    case DSP_Rotate_270:
                    {
                        tmp.width = bmp->height;
                        tmp.height = bmp->width;
                        //qDebug() << tmp.width << tmp.height << shift;
                        for (uint32_t x = 0; x < width; x++)
                            for (uint32_t y = 0; y < bmp->height; y++)
                               if (bmp->data[x/8+(y*width/8)] & (0x80 >> (x%8))) tmp.data[y/8+((width-shift-x-1)*width/8)] |= (0x80 >> (y%8));
                        break;
                    }
                }//--SWITCH--
                dsp[handle-1].init->driver->drawBitmap(dsp[handle-1].init->drvSettings,x,y,&tmp,dsp[handle-1].foreColor);
                free(tmp.data);
            }
        }
        else dsp[handle-1].init->driver->drawBitmap(dsp[handle-1].init->drvSettings,x,y,bmp,dsp[handle-1].foreColor);
    }
}
